...
int StdioBaseScanf(struct ParseState *Parser, FILE *Stream, char *StrIn, char *Format, struct StdVararg *Args)
{
    ...
    if (Stream != NULL)
		result = fscanf(Stream, Format, ScanfArg[0], ScanfArg[1], ScanfArg[2], ScanfArg[3], ScanfArg[4], ScanfArg[5], ScanfArg[6], ScanfArg[7], ScanfArg[8], ScanfArg[9]);
    else
		result = sscanf(StrIn, Format, ScanfArg[0], ScanfArg[1], ScanfArg[2], ScanfArg[3], ScanfArg[4], ScanfArg[5], ScanfArg[6], ScanfArg[7], ScanfArg[8], ScanfArg[9]);

	for (ArgCount = 0; ArgCount < Args->NumArgs; ArgCount++)
	{
		ThisArg = (struct Value *)((char *)ThisArg + MEM_ALIGN(sizeof(struct Value) + TypeStackSizeValue(ThisArg)));
		write_affectation((balisage_fonction) ? identifiant : identifiant++, Parser->Line, Parser->FileName, ThisArg->Typ, ThisArg);		
	}
	return result;
}
...
void StdioPutc(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
	...
	/*write_in_xmlfile("%s<affectation id=\"%d\" line=\"%d\" file=\"%s\" type=\"%s\" name=\"%s\">%s</affectation>\n", tabulation, (balisage_fonction)?identifiant : identifiant++, Parser->Line, Parser->FileName, get_type_name(Param[0]->Typ), Param[0]->name, get_object_value(Param[1]));*/
}
...
void StdioPrintf(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
    ...
    ReturnValue->Val->Integer = StdioBasePrintf(Parser, stdin, NULL, 0, Param[0]->Val->Pointer, &PrintfArgs);
}

void StdioVprintf(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
	ReturnValue->Val->Integer = StdioBasePrintf(Parser, stdin, NULL, 0, Param[0]->Val->Pointer, Param[1]->Val->Pointer);
}
...
void StdioVfprintf(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
	...
	/*write_in_xmlfile("%s<affectation id=\"%d\" line=\"%d\" file=\"%s\" type=\"%s\" name=\"%s\">%s</affectation>\n", tabulation, (balisage_fonction)?identifiant : identifiant++, Parser->Line, Parser->FileName, get_type_name(Param[0]->Typ), Param[0]->name, get_object_value(Param[0]));*/
}
...
void StdioSprintf(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
	...
	write_affectation((balisage_fonction) ? identifiant : identifiant++, Parser->Line, Parser->FileName, Param[0]->Typ, Param[0]);
}
