...
long ExpressionAssignInt(struct ParseState *Parser, struct Value *DestValue, long FromInt, int After)
{
	...
	if (DestValue->Refdest != NULL && strcmp("__exit_value", DestValue->Refdest->Name))
		write_affectation(balisage_fonction ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName), DestValue->Typ, DestValue);
	return Result;
}
...
double ExpressionAssignFP(struct ParseState *Parser, struct Value *DestValue, double FromFP)
{
	...
	write_affectation(balisage_fonction ? identifiant : identifiant++, Parser->Line, Parser->FileName, DestValue->Typ, DestValue);
	return FromFP;
}
#endif
...
void ExpressionStackPushDereference(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *DereferenceValue)
{
	...
	ValueLoc->Refdest = DereferenceValue->Refdest;
	ExpressionStackPushValueNode(Parser, StackTop, ValueLoc);
}
...
{
	struct ValueType *PointedToType = ToValue->Typ->FromType, *PointedFromType;
	if (FromValue->Typ == ToValue->Typ || FromValue->Typ == VoidPtrType || (ToValue->Typ == VoidPtrType && FromValue->Typ->Base == TypePointer))
	{
		PointedToType = ToValue->Typ;
		PointedFromType = FromValue->Typ;
		while (PointedToType->FromType != NULL && PointedFromType->FromType != NULL)
		{
			PointedToType->ArraySize = PointedFromType->ArraySize;
			PointedToType = PointedToType->FromType;
			PointedFromType = PointedFromType->FromType;
		}

		ToValue->Val->Pointer = FromValue->Val->Pointer;      /* plain old pointer assignment */
		ToValue->Typ->SizeOFBis = FromValue->Typ->SizeArrayOf;
		if (PointedToType->SizeOFBis != 0)
			ToValue->Typ->SizeArrayOf = ToValue->Typ->SizeOFBis / PointedToType->Sizeof;
		ToValue->Refsource = FromValue->Refdest;
		if (ToValue->Refdest == NULL)
			ToValue->Refdest = ToValue->Refsource;
	}
	else if (FromValue->Typ->Base == TypeArray && (PointedToType == FromValue->Typ->FromType || ToValue->Typ == VoidPtrType))
	{
		...
		ToValue->Typ->ArraySize = FromValue->Typ->ArraySize;

		if (ToValue->Refdest == NULL)
		{
			strcpy(ToValue->Typ->TypeId, "array");
			ToValue->Refdest = FromValue->Refdest;
		}
		else
			ToValue->Refsource = FromValue->Refdest;
	}
	...
	else if (IS_NUMERIC_COERCIBLE(FromValue) && ExpressionCoerceInteger(FromValue) == 0)
	{
		...
		ToValue->Refsource = FromValue->Refdest;
		if (ToValue->Refsource != NULL && VariableDefined(ToValue->Refsource->Name))
			write_affectation((balisage_fonction) ? identifiant : identifiant++, Parser->Line, Parser->FileName, ToValue->Typ, ToValue);
	}
	...
	else if (ToValue->Typ->Base == TypePointer && (FromValue->Typ->Base == TypePointer || FromValue->Typ->Base == TypeArray))
	{
		PointedToType = ToValue->Typ;
		PointedFromType = FromValue->Typ;
		while (PointedToType->FromType != NULL && PointedFromType->FromType != NULL)
		...
		if (PointedToType->Base != PointedFromType->Base)
			AssignFail(Parser, "%t from %t", ToValue->Typ, FromValue->Typ, 0, 0, FuncName, ParamNo);

		ToValue->Val->Pointer = FromValue->Val->Pointer;
		ToValue->Typ->SizeArrayOf = FromValue->Typ->SizeArrayOf;
		ToValue->Refsource = FromValue->Refdest;
		if (FromValue->Typ->Base == TypeArray)
			ToValue->Val->Pointer = (void *)&FromValue->Val->ArrayMem[0];
	}
	else
		AssignFail(Parser, "%t from %t", ToValue->Typ, FromValue->Typ, 0, 0, FuncName, ParamNo);
...
void ExpressionAssign(struct ParseState *Parser, struct Value *DestValue, struct Value *SourceValue, int Force, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
	...
	case TypeArray:
		...
		DestValue->Refsource = SourceValue->Refdest;
		break;
	...
	case TypeUnion:
		...
		DestValue->Refdest = SourceValue->Refdest;
		break;
...
void ExpressionPrefixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *TopValue)
{
	...
	case TokenAmpersand:
		...
		Result->Refdest = TopValue->Refdest;
...
void ExpressionPostfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *TopValue)
{
	...
	else if (TopValue->Typ->Base == TypePointer)
	{
		...
		Refss = TopValue->Refdest;
		StackValue = ExpressionStackPushValueByType(Parser, StackTop, TopValue->Typ);
		StackValue->Val->Pointer = OrigPointer;
		StackValue->Refdest = Refss;
	}
	else
		ProgramFail(Parser, "invalid operation");
}
...
void ExpressionInfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *BottomValue, struct Value *TopValue)
{
	...
	if (Op == TokenLeftSquareBracket)
	{
		...
		Refs = calloc(1, sizeof(struct Ident));
		memcpy(Refs, BottomValue->Refdest, sizeof(struct Ident));
		sprintf(Name, "%s[%d]", BottomValue->Refdest->Name, ArrayIndex);
	...
	else if (IS_NUMERIC_COERCIBLE(TopValue) && IS_NUMERIC_COERCIBLE(BottomValue))
	{
		...
		BottomValue->Refsource = (TopValue->Refdest != NULL) ? TopValue->Refdest : BottomValue->Refsource;
		switch (Op)
		{
	...
	else if (Op == TokenAssign)
	{
		...
		ExpressionAssign(Parser, BottomValue, TopValue, FALSE, NULL, 0, FALSE);
		ExpressionStackPushValueNode(Parser, StackTop, BottomValue);
		BottomValue->Refsource = TopValue->Refdest;
		write_affectation((balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName),
			BottomValue->Typ, BottomValue);
	}
...
void ExpressionGetStructElement(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Token)
{
	...
	if (Parser->Mode == RunModeRun)
	{
		...
		struct Value *Result;
		Refs = calloc(1, sizeof(struct Ident));
		memcpy(Refs, (*StackTop)->Val->Refdest, sizeof(struct Ident));
		...
		*StackTop = (*StackTop)->Next;
		...
		ExpressionStackPushValueNode(Parser, StackTop, Result);
		sprintf(name, "%s.%s", Refs->Name, Ident->Val->Identifier);
		(*StackTop)->Val->Refdest = Refs;
		(*StackTop)->Val->Refdest->Name = _strdup(name);
	}
}
			...
			else
			{
				if (Parser->Mode == RunModeRun && Precedence < IgnorePrecedence)
				{
					...
					else
					{
						...
						StackTop->Val->Refdest = VariableValue->Refdest;
					}
...
void ExpressionParseFunctionCall(struct ParseState *Parser, struct ExpressionStack **StackTop, const char *FuncName, int RunIt)
{
	...
	if (RunIt /*Mode Running*/)
	{
		if (strcmp("main", FuncName) == 0)
		{
			write_in_xmlfile("%s<stacklist id=\"%d\" line=\"%d\" file=\"%s\" typeid=\"function\" name=\"%s\" type=\"",
				tabulation, (balisage_fonction) ? identifiant : identifiant++, RefsMain.OrderNum, RefsMain.Name, FuncName);
			write_type_name(FuncValue->Val->Typ);
			write_in_xmlfile("\">\n");
		}
		else
		{
			write_in_xmlfile("%s<stacklist id=\"%d\" line=\"%d\" file=\"%s\" typeid=\"function\" name=\"%s\" type=\"",
				tabulation, (balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName), FuncName);
			write_type_name(FuncValue->Val->Typ);
			write_in_xmlfile("\">\n");
		}
	}
	...
	if (RunIt)
	{
		...
		if (FuncValue->Val->FuncDef.Intrinsic == NULL)
		{
			...
			for (Count = 0; Count < FuncValue->Val->FuncDef.NumParams; Count++)
			{
				...
				write_paramater((balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName),
					FuncValue->Val->FuncDef.ParamType[Count], FuncValue->Val->FuncDef.ParamName[Count], ParamArray[Count]);
			}
			...
			write_in_xmlfile("%s</stacklist>\n", tabulation);
		}
		else
		{
			FuncValue->Val->FuncDef.Intrinsic(Parser, ReturnValue, ParamArray, ArgCount);
			write_return((balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName),
				ReturnValue->Typ, ReturnValue);
			tabulation[strlen(tabulation) - 3] = '\0';
			write_in_xmlfile("%s</stacklist>\n", tabulation);
		}
