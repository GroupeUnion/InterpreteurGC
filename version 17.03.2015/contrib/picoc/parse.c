...
void ParseDeclarationAssignment(struct ParseState *Parser, struct Value *NewVariable, int DoAssignment)
{
	...
	else
	{
		...
		if (Parser->Mode == RunModeRun && DoAssignment)
		{
			...
			VariableStackPop(Parser, CValue);
			NewVariable->Refsource = CValue->Refdest;
		}
	}
...
int ParseDeclaration(struct ParseState *Parser, enum LexToken Token)
{
	...
	do
	{
		...
		if (Identifier != StrEmpty)
		{
			...
			{
				if (!VariableDefined(Identifier))
				{
					balisage_fonction = 1;
					if (strcmp("main", Identifier) == 0)
					{
						RefsMain.Name = get_filename(Parser->FileName);
						RefsMain.OrderNum = Parser->Line;
					}
					//write_in_xmlfile("<fonction id=\"%d\" line=\"%d\" file=\"%s\" return=\"%s\" name=\"%s\">", Parser->Line, Parser->FileName, get_type_name(Typ), Identifier);
					ParseFunctionDefinition(Parser, Typ, Identifier);
					/*if (val->Val->FuncDef.NumParams > 0)
					write_in_xmlfile("\n");
					for (Count = 0; Count < val->Val->FuncDef.NumParams; Count++)
					{
					write_in_xmlfile("<param type=\"%s\" name=\"%s\"></param>\n",
					get_type_name(val->Val->FuncDef.ParamType[Count]),
					val->Val->FuncDef.ParamName[Count]);
					}
					write_in_xmlfile("</fonction>\n");*/
					balisage_fonction = 0;
				}
				else
					ParseFunctionDefinition(Parser, Typ, Identifier);
			...
			else
			{
				...
				if (FirstVisit)
				{
					write_variable((balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName),
						Typ, Identifier, NULL);
				}
				if (LexGetToken(Parser, NULL, FALSE) == TokenAssign)
				{
					...
					LexGetToken(Parser, NULL, TRUE);
					id = (balisage_fonction) ? identifiant : identifiant++;					
					ParseDeclarationAssignment(Parser, NewVariable, !IsStatic || FirstVisit);
					if (NewVariable != NULL)
					{
						write_variable(id, Parser->Line, get_filename(Parser->FileName),
							Typ, Identifier, id + 1 != identifiant ? NULL : NewVariable);
						if (id + 1 != identifiant)
						{
		...
		if (BasicType->Base == TypeStruct)
		write_in_xmlfile("%s<struct id=\"%d\" line=\"%d\" file=\"%s\" name=\"%s\">\n%s%s</struct>\n", tabulation, (balisage_fonction)?identifiant : identifiant++,
		Parser->Line, Parser->FileName, Typ->Identifier, get_info_member(Typ->LMembres), tabulation);
...
enum ParseResult ParseStatement(struct ParseState *Parser, int CheckTrailingSemicolon)
{
	...
	case TokenIf:
		...
		write_in_xmlfile("%s<stacklist id=\"%d\" line=\"%d\" file=\"%s\" typeid=\"if\">\n", tabulation, (balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName));
		strcat(tabulation, "   ");
		Condition = ExpressionParseInt(Parser);
		...
		{
			if (!Condition)
				write_in_xmlfile("%s<stacklist id=\"%d\" line=\"%d\" file=\"%s\" typeid=\"goto\"></stacklist>\n", tabulation, (balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName));
			LexGetToken(Parser, NULL, TRUE);
			if (ParseStatementMaybeRun(Parser, !Condition, TRUE) != ParseResultOk)
				ProgramFail(Parser, "statement expected");
		}
	...
	case TokenWhile:
	{
		...
		ParserCopyPos(&PreConditional, Parser);
		ligneWhile = Parser->Line;
		write_in_xmlfile("%s<stacklist id=\"%d\" line=\"%d\" file=\"%s\" typeid=\"while\">\n", tabulation, (balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName));
		strcat(tabulation, "   ");
		do
		{
			ParserCopyPos(Parser, &PreConditional);
			...
			if (Parser->Mode == RunModeContinue)
				Parser->Mode = PreMode;
			if (Parser->Mode == RunModeRun && Condition)
				write_in_xmlfile("%s<stacklist id=\"%d\" line=\"%d\" file=\"%s\" typeid=\"goto\"></stacklist>\n", tabulation, (balisage_fonction) ? identifiant : identifiant++, ligneWhile, get_filename(Parser->FileName));
	...
	case TokenFor:
		write_in_xmlfile("%s<stacklist id=\"%d\" line=\"%d\" file=\"%s\" typeid=\"for\">\n", tabulation, (balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName));
		strcat(tabulation, "   ");
		ParseFor(Parser);
		tabulation[strlen(tabulation) - 3] = '\0';
		write_in_xmlfile("%s</stacklist>\n", tabulation);
	...
	case TokenSwitch:
		write_in_xmlfile("%s<stacklist id=\"%d\" line=\"%d\" file=\"%s\" typeid=\"switch\">\n", tabulation, (balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName));
		strcat(tabulation, "   ");
		if (LexGetToken(Parser, NULL, TRUE) != TokenOpenBracket)
			ProgramFail(Parser, "'(' expected");
	...
	case TokenCase:
		...
		{
			Parser->Mode = RunModeRun;
			write_in_xmlfile("%s<stacklist id=\"%d\" line=\"%d\" file=\"%s\" typeid=\"case\">\n", tabulation, (balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName));
			strcat(tabulation, "   ");
		}
	...
	case TokenBreak:
		if (Parser->Mode == RunModeRun)
		{
			...
			write_in_xmlfile("%s<stacklist id=\"%d\" line=\"%d\" file=\"%s\" typeid=\"break\"></stacklist>\n", tabulation, (balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName));
			if (Parser->SearchLabel)
			{				
				tabulation[strlen(tabulation) - 3] = '\0';
				write_in_xmlfile("%s</stacklist>\n", tabulation);
			}
		...
		if (Parser->Mode == RunModeRun)
		{
			if (TopStackFrame->ReturnValue->Typ->Base != TypeVoid)
			{
				...
				write_return((balisage_fonction) ? identifiant : identifiant++, Parser->Line, get_filename(Parser->FileName),
					TopStackFrame->ReturnValue->Typ, TopStackFrame->ReturnValue);
				if (Parser->SearchLabel)
				{
					tabulation[strlen(tabulation) - 3] = '\0';
					write_in_xmlfile("%s</stacklist>\n", tabulation);
