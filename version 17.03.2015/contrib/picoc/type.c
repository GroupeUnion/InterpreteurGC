...
struct ValueType *TypeGetMatching(struct ParseState *Parser, struct ValueType *ParentType, enum BaseType Base, int ArraySize, const char *Identifier, int AllowDuplicates)
{
	...
	ThisType = TypeAdd(Parser, ParentType, Base, ArraySize, Identifier, Sizeof, AlignBytes);
	if (!ParentType->SizeArrayOf)
		ThisType->SizeArrayOf += ArraySize + ParentType->SizeArrayOf;
	else 
		ThisType->SizeArrayOf = ArraySize * ParentType->SizeArrayOf;
	ParentType->SizeArrayOf = 0;
	return ThisType;
}
...
void TypeInit()
{
	...
	TypeAddBaseType(&DoubleType, TypeDouble, sizeof(double), (char *)&da.y - &da.x);
...
void TypeParseStruct(struct ParseState *Parser, struct ValueType **Typ, int IsStruct)
{
    ...
    *Typ = TypeGetMatching(Parser, &UberType, IsStruct ? TypeStruct : TypeUnion, 0, StructIdentifier, Token != TokenLeftBrace);
    ...
    do {
		...
		member_ptr = add_member_name(&((*Typ)->LMembres), MemberIdentifier, MemberValue);
        if (IsStruct)
        {
			...
			member_ptr->NextMemberOffset = (*Typ)->Sizeof;
    ...
    if (((*Typ)->Sizeof & (AlignBoundary-1)) != 0)
        (*Typ)->Sizeof += AlignBoundary - ((*Typ)->Sizeof & (AlignBoundary-1));	
	write_in_xmlfile("%s<var id=\"%d\" line=\"%d\" file=\"%s\" type=\"struct %s\" name=\"\" typeid=\"struct\">\n", 
		tabulation, (balisage_fonction) ? identifiant : identifiant++, 
		Parser->Line, get_filename(Parser->FileName), (*Typ)->Identifier);
	write_structure_member((*Typ)->LMembres);
...
int TypeParseFront(struct ParseState *Parser, struct ValueType **Typ, int *IsStatic)
{
    ...
    *Typ = NULL;
    ...
    switch (Token)
    {
		...
		case TokenDoubleType: *Typ = &DoubleType; break;
#endif
